...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'franka_pick_dyn_labyrinth.xml')
...
class FrankaFetchPickDynLabyrinthEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=20):
        ...
        super(FrankaFetchPickDynLabyrinthEnv, self).__init__(
    ...
    def step(self, action):
        self.move_obstacles()
        return super(FrankaFetchPickDynLabyrinthEnv, self).step(action)
    ...
    def _step_callback(self):
        ...
        if self.block_object_in_gripper and self.block_gripper:
            self.sim.data.set_joint_qpos('panda0_finger_joint1', 0.026)
            self.sim.data.set_joint_qpos('panda0_finger_joint2', 0.026)
            self.sim.forward()
    ...
    def _reset_sim(self):
        ...
        if self.block_object_in_gripper:
            ...
            if self.block_gripper:
                ...
                self.sim.data.set_joint_qpos('panda0_finger_joint1', 0.05)
                self.sim.data.set_joint_qpos('panda0_finger_joint2', 0.05)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FrankaFetchPickDynLabyrinthEnv, self).render(mode, width, height)
