...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'franka_pick_dyn_sqr_obstacle.xml')
...
class FrankaFetchPickDynSqrObstacleEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=20):
        ...
        super(FrankaFetchPickDynSqrObstacleEnv, self).__init__(
    ...
    def step(self, action):
        ...
        return super(FrankaFetchPickDynSqrObstacleEnv, self).step(action)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FrankaFetchPickDynSqrObstacleEnv, self).render(mode, width, height)
