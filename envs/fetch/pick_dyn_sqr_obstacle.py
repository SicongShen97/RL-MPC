...
MODEL_XML_PATH = os.path.join(os.getcwd(), 'envs', 'assets', 'fetch', 'pick_dyn_sqr_obstacle.xml')
...
class FetchPickDynSqrObstacleEnv(robot_env.RobotEnv, gym.utils.EzPickle):
    def __init__(self, reward_type='sparse', n_substeps=20):
        ...
        super(FetchPickDynSqrObstacleEnv, self).__init__(
    ...
    def step(self, action):
        ...
        return super(FetchPickDynSqrObstacleEnv, self).step(action)
    ...
    def render(self, mode='human', width=1080, height=1080):
        return super(FetchPickDynSqrObstacleEnv, self).render(mode, width, height)
