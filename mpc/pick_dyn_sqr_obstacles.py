...
def objective(z, p):
    ...
    return 10 * ((x[0] - p[0]) ** 2 + (x[1] - p[1]) ** 2) + \
           0.01 * acc_x_rel ** 2 + 0.01 * acc_y_rel ** 2 + 100 * u[3] ** 2
...
def inequality_constraints(z, p):
    ...
    return casadi.vertcat(
        casadi.sqrt((p_x - x_o1) ** 2 + (p_y - y_o1) ** 2) + u[3],
        casadi.sqrt((p_x - x_o2) ** 2 + (p_y - y_o2) ** 2) + u[3],
        p_x + u[3]
    )
...
def generate_pathplanner(create=True, path=''):
    ...
    model.hl = np.array([0.05 + 0.06, 0.05 + 0.06,       0])
    ...
    # Set solver options
    codeoptions = forcespro.CodeOptions('FORCESNLPsolver1_1')
    ...
    codeoptions.nlp.integrator.Ts = 0.01  # determines the absolute time between two integration intervals
    codeoptions.nlp.integrator.nodes = 5  # defines the number of intermediate integration nodes within that
    ...
    if create:
        ...
        solver = forcespro.nlp.Solver.from_directory(path + "FORCESNLPsolver1_1")
...
def main():
    ...
    goal = np.array([1.45, 0.43, 0.44])
    t = 0
    dt = codeoptions.nlp.integrator.Ts
    vels = np.array([0.5, 0.7])
    shifts = np.array([0.0, 0.0])
    pos_difs = np.array([0.22, 0.22])

    stat_obstacles = []
    dyn_obstacles = [[1.3, 0.60, 0.435, 0.03, 0.03, 0.03], [1.3, 0.80, 0.435, 0.03, 0.03, 0.03]]
