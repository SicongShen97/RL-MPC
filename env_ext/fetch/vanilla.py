...
class VanillaGoalEnv():
	def __init__(self, args):
		self.args = args
		self.env = gym.make(args.env, n_substeps=args.env_n_substeps)
		...
		self.block_object_in_gripper = self.env.env.block_object_in_gripper
		...
		self.reset_sim = self.env.env.env._reset_sim
	...
	def get_obs(self):
		...
		return self.env.env.env._get_obs()
	...
	def step(self, action):
		...
		reward = self.compute_reward((obs['achieved_goal'], self.prev_obs['achieved_goal']), obs['desired_goal'])
	...
	def reset(self):
		self.reset_ep()
		self.prev_obs = (self.env.reset()).copy()
	...
	def generate_goal(self):
		# mostly for use in GHGG learner
