...
typedef FrankaFORCESNLPsolver_dyn_sqr_obsts_float solver_float;
...
void copyCArrayToM_FrankaFORCESNLPsolver_dyn_sqr_obsts(FrankaFORCESNLPsolver_dyn_sqr_obsts_float *src, double *dest, solver_int32_default dim) 
{
...
void copyCValueToM_FrankaFORCESNLPsolver_dyn_sqr_obsts(FrankaFORCESNLPsolver_dyn_sqr_obsts_float* src, double* dest)
{
    *dest = (double)*src;
}

void copyMArrayToC_FrankaFORCESNLPsolver_dyn_sqr_obsts(double *src, FrankaFORCESNLPsolver_dyn_sqr_obsts_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
        *dest++ = (FrankaFORCESNLPsolver_dyn_sqr_obsts_float) (*src++) ;
    }
}

void copyMValueToC_FrankaFORCESNLPsolver_dyn_sqr_obsts(double * src, FrankaFORCESNLPsolver_dyn_sqr_obsts_float * dest)
{
	*dest = (FrankaFORCESNLPsolver_dyn_sqr_obsts_float) *src;
}
...
void mexFunction( solver_int nlhs, mxArray *plhs[], solver_int nrhs, const mxArray *prhs[] )  
{
    ...
    arr = prhs[0];
    copyMArrayToC_FrankaFORCESNLPsolver_dyn_sqr_obsts(mxGetPr(arr), z, nvar);
    if (nrhs > 1)
	{
        arr = prhs[1];
        if ( !mxIsEmpty(arr) )
        {
            copyMArrayToC_FrankaFORCESNLPsolver_dyn_sqr_obsts(mxGetPr(arr), p, dimp);
        }
	}   
	...
	copyCArrayToM_FrankaFORCESNLPsolver_dyn_sqr_obsts(obj, mxGetPr(obj_mex), 1);
	copyCArrayToM_FrankaFORCESNLPsolver_dyn_sqr_obsts(jacobj, mxGetPr(gradobj_mex), nvar);
	plhs[0] = obj_mex;
	plhs[1] = gradobj_mex;
