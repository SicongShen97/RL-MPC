...
#include "../include/FrankaFORCESNLPsolver_5mm_memory.h"
...
extern solver_int32_default (FrankaFORCESNLPsolver_5mm_float *x, FrankaFORCESNLPsolver_5mm_float *y, FrankaFORCESNLPsolver_5mm_float *l, FrankaFORCESNLPsolver_5mm_float *p, FrankaFORCESNLPsolver_5mm_float *f, FrankaFORCESNLPsolver_5mm_float *nabla_f, FrankaFORCESNLPsolver_5mm_float *c, FrankaFORCESNLPsolver_5mm_float *nabla_c, FrankaFORCESNLPsolver_5mm_float *h, FrankaFORCESNLPsolver_5mm_float *nabla_h, FrankaFORCESNLPsolver_5mm_float *hess, solver_int32_default stage, solver_int32_default iteration, solver_int32_default threadID);
FrankaFORCESNLPsolver_5mm_extfunc pt2function_FrankaFORCESNLPsolver_5mm = &;


/* Some memory for mex-function */
static FrankaFORCESNLPsolver_5mm_params params;
static FrankaFORCESNLPsolver_5mm_output output;
static FrankaFORCESNLPsolver_5mm_info info;
static FrankaFORCESNLPsolver_5mm_mem * mem;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1)
	{
		mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help FrankaFORCESNLPsolver_5mm_mex' for details.");
	}    
	if (nlhs > 3) 
	{
    ...
    if (mem == NULL)
    {
        mem = FrankaFORCESNLPsolver_5mm_internal_mem(0);
    }
	...
	{
    mexErrMsgTxt("PARAMS.x0 must be of size [40 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.x0,40);
	...
	#if SET_PRINTLEVEL_FrankaFORCESNLPsolver_5mm > 0
	...
	exitflag = FrankaFORCESNLPsolver_5mm_solve(&params, &output, &info, mem, fp, pt2function_FrankaFORCESNLPsolver_5mm);
	...
	copyCArrayToM_double( output.x01, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x01", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x02, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x02", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x03, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x03", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x04, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x04", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x05, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x05", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x06, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x06", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x07, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x07", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x08, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x08", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x09, mxGetPr(outvar), 4);
	mxSetField(plhs[0], 0, "x09", outvar);

	outvar = mxCreateDoubleMatrix(4, 1, mxREAL);
	copyCArrayToM_double( output.x10, mxGetPr(outvar), 4);
