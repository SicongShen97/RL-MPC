...
typedef FrankaFORCESNLPsolver_5mm_float solver_float;
...
void copyCArrayToM_FrankaFORCESNLPsolver_5mm(FrankaFORCESNLPsolver_5mm_float *src, double *dest, solver_int32_default dim) 
{
...
void copyCValueToM_FrankaFORCESNLPsolver_5mm(FrankaFORCESNLPsolver_5mm_float* src, double* dest)
{
    *dest = (double)*src;
}

void copyMArrayToC_FrankaFORCESNLPsolver_5mm(double *src, FrankaFORCESNLPsolver_5mm_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
        *dest++ = (FrankaFORCESNLPsolver_5mm_float) (*src++) ;
    }
}

void copyMValueToC_FrankaFORCESNLPsolver_5mm(double * src, FrankaFORCESNLPsolver_5mm_float * dest)
{
	*dest = (FrankaFORCESNLPsolver_5mm_float) *src;
}
...
{
    const solver_int nvarArr[NSTAGES] = {4, 4, 4, 4, 4, 4, 4, 4, 4, 4}; 
    ...
    const solver_int dimlArr[NSTAGES] = {2, 3, 3, 3, 3, 3, 3, 3, 3, 3};
...
void mexFunction( solver_int nlhs, mxArray *plhs[], solver_int nrhs, const mxArray *prhs[] )  
{
    ...
    arr = prhs[0];
    copyMArrayToC_FrankaFORCESNLPsolver_5mm(mxGetPr(arr), z, nvar);
    if (nrhs > 1)
	{
        arr = prhs[1];
        if ( !mxIsEmpty(arr) )
        {
            copyMArrayToC_FrankaFORCESNLPsolver_5mm(mxGetPr(arr), p, dimp);
        }
	}   
	...
	copyCArrayToM_FrankaFORCESNLPsolver_5mm(obj, mxGetPr(obj_mex), 1);
	copyCArrayToM_FrankaFORCESNLPsolver_5mm(jacobj, mxGetPr(gradobj_mex), nvar);
	plhs[0] = obj_mex;
	plhs[1] = gradobj_mex;
