...
def call_multiple_method(hostname_list: list, port: int, method, payload=None, endpoint="mios/core", timeout=100, silent=False):
    try:
        request = {
            "method": method,
            "request": payload
        ...
        asyncio.set_event_loop(asyncio.new_event_loop())
        send_task_list = [send(hostname, request=request, port=port, endpoint=endpoint, timeout=timeout, silent=silent)
                          for hostname in hostname_list]
        return asyncio.get_event_loop().run_until_complete(asyncio.wait(send_task_list))
    except socket.gaierror as e:
        print(e)
        print("Hostname: " + hostname_list + ", port:" + str(port) + ", endpoint: " + endpoint)
        return None
...
def start_multiple_task(hostname_list: list, task: str, parameters={}, queue=False):
    payload = {
        "task": task,
        "parameters": parameters,
        "queue": queue
    }
    return call_multiple_method(hostname_list, 12000, "start_task", payload)
